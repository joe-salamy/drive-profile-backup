...
class HarnessWorktreeFlow:
    ...
    def _validate_runtime(self, *, resume: bool = False) -> tuple[Path, Path, str, str]:
        ...
        self.adapter.prepare_permissions(
            [self.repo / self.harness_dir, self.git.git_common_dir(self.repo)]
        )
        if not resume:
    ...
    def _validate_saved_plan_reservation(self, plan: Path, run_id: str | None) -> None:
        ...
        if canonical_path(plan.parent, must_exist=True) != expected:
            raise FlowError(
                "Saved plan is not in its generated workflow reservation directory."
            )
        entries = regular_directory_entries(expected)
        if {entry.name for entry in entries} != {"plan.md"}:
            raise FlowError(
                "Saved plan reservation contains unexpected or unsafe entries."
            )
        ...
        *,
        ...
        if plan_digest is not None and self.state_store.root.exists():
            for candidate_dir in regular_directory_entries(self.state_store.root):
                ...
                if (
                    candidate.plan_sha256 == plan_digest
                    and candidate.stage is WorkflowStage.COMPLETE
                ):
                    raise FlowError(
        ...
        for suffix in range(1, 1000):
            ...
            worktree = (
                self.repo.parent / f"{self.repo.name}-{candidate_slug}"
            ).resolve()
            run_id = (
                saved_run_id
            ...
            if state_dir.exists():
                if saved_run_id != run_id:
                    continue
                raise FlowError(
                    f"Workflow state already exists; use --resume: {state_dir}"
                )
    ...
    def _new_state(
        self, names: Names, plan: Path, title: str, feature_base_commit: str
    ) -> WorkflowState:
    ...
    def _transition(
        self, state: WorkflowState, stage: WorkflowStage, **changes: object
    ) -> WorkflowState:
        assert self.state_store is not None
    ...
    def start_log(self, worktree: Path, run_id: str) -> None:
        ...
        self.log_event(
            "workflow_log_started", log_file=str(self.log_file), run_id=run_id
        )
    ...
    def continue_log(self, worktree: Path, run_id: str) -> None:
        ...
        ensure_directory(self.log_file.parent, mode=0o700)
        self.log_event(
            "workflow_log_continued", log_file=str(self.log_file), run_id=run_id
        )
    ...
    def log_event(self, event: str, **fields: object) -> None:
        ...
        previous = (
            read_bytes_bounded(self.log_file, max_bytes=10 * 1024 * 1024)
            if optional_regular_file(self.log_file, label="workflow log")
            else b""
        )
    ...
    def _allocate_feature(self, state: WorkflowState, names: Names) -> WorkflowState:
        ...
        branch_entries = [
            ...
            if entry.branch == names.feature_branch
        ]
        if state.stage is WorkflowStage.FEATURE_ALLOCATED:
            ...
            if entries or branch_entries:
                if (
                    not entries
                    or branch_entries != entries
                    or entries[0].branch != names.feature_branch
                ):
                    raise FlowError(
                        "Feature allocation is partial or mismatched; refusing to repair it."
                    )
            else:
                self.runner.run(
                    [
                        ...
                        "add",
                        "-b",
                        names.feature_branch,
                        "--",
                        str(path),
                        state.feature_base_commit,
                    ],
                    self.repo,
                )
            ...
            if self.git.head(path) != state.feature_base_commit:
                raise FlowError(
                    "Allocated feature worktree does not start at the recorded base commit."
                )
    ...
    def _validate_pre_copy_plan(self, state: WorkflowState) -> None:
        ...
        if sha256_file(source) != state.plan_sha256:
            raise FlowError(
                "Source plan digest differs from workflow state; refusing to continue."
            )

    def _copy_plan_stage(
        self, state: WorkflowState, plan: Path
        ...
        if state.stage is WorkflowStage.FEATURE_WORKTREE_CREATED:
            if optional_regular_file(target, label="copied plan"):
                if sha256_file(target) != state.plan_sha256:
                    raise FlowError(
                        "Copied plan digest differs from recorded plan_sha256."
                    )
            else:
            ...
            state = self._transition(
                state, WorkflowStage.PLAN_COPIED, plan_path=str(target)
            )
        elif state.plan_path != str(target):
            raise FlowError(
                "Workflow state plan path does not match the generated feature plan path."
            )
    ...
    def _verify_plan(self, state: WorkflowState) -> None:
        ...
        if sha256_file(path) != state.plan_sha256:
            raise FlowError(
                "Copied plan digest differs from workflow state; refusing to continue."
            )
    ...
    def _run_implementation(self, state: WorkflowState, plan: Path) -> WorkflowState:
        ...
        self.print_checkpoint(
            "start", "Implementation", (("worktree", worktree), ("plan", plan))
        )
        self.adapter.execute(
            worktree,
            implementation_prompt(worktree, plan, self.harness_dir),
        ...
        if (
            self.git.count_commits_since(
                state.feature_base_commit, state.feature_branch, worktree
            )
            <= 0
        ):
            raise FlowError(
                "Implementation did not create a commit after feature_base_commit."
            )
        self.git.require_changed_since(
            state.base_branch, state.feature_branch, worktree
        )
        self.git.require_clean_except_artifacts(worktree, phase="Implementation")
        head = self.git.head(worktree)
        state = self._transition(
            state, WorkflowStage.IMPLEMENTATION_COMPLETE, implementation_head=head
        )
    ...
    def _run_audit(self, state: WorkflowState, plan: Path) -> WorkflowState:
        ...
        if state.stage is WorkflowStage.IMPLEMENTATION_COMPLETE:
            ...
            state = self._transition(
                state, WorkflowStage.AUDIT_STARTED, audit_start_head=start_head
            )
            ...
            if state.audit_head != current:
                state = self._transition(
                    state, WorkflowStage.AUDIT_STARTED, audit_start_head=current
                )
            else:
                self.git.require_clean_except_artifacts(
                    worktree, phase="Completed audit"
                )
        ...
        self._verify_plan(state)
        self.print_checkpoint(
            "start", "Audit", (("worktree", worktree), ("plan", plan))
        )
        self.adapter.execute(
            worktree,
            audit_prompt(worktree, plan, self.harness_dir, post_conflict=False),
            phase="audit",
        )
        self._verify_plan(state)
    ...
    def _merge_mode_resume(self, state: WorkflowState) -> WorkflowState | None:
        ...
        if state.stage is WorkflowStage.STOPPED_BEFORE_MERGE:
            if requested is None:
                self.print_checkpoint(
                    "stop", "Stopped before merge", (("resume", self.resume_command()),)
                )
                return None
            if requested not in {"squash", "no-ff"}:
                raise FlowError(
                    "A stopped resume requires --merge-mode squash or no-ff."
                )
            state = self._transition(
                state,
                WorkflowStage.AUDIT_STARTED,
                merge_mode=requested,
                archive_commit=None,
            )
            return state
    ...
    def _allocate_integration(self, state: WorkflowState) -> tuple[WorkflowState, Path]:
        ...
        if state.stage is WorkflowStage.AUDIT_COMPLETE:
            ...
            for suffix in range(1, 1000):
                if (
                    ...
                    or path.exists()
                    or any(entry.path == path for entry in self.git.worktrees())
                ):
                    branch = f"integration/{state.slug}-{stamp}-{suffix}"
                    path = (
                        self.repo.parent
                        / f"{self.repo.name}-integrate-{state.slug}-{stamp}-{suffix}"
                    ).resolve()
                    continue
                break
        ...
        if state.stage is WorkflowStage.INTEGRATION_ALLOCATED:
            ...
            if entries:
                ...
                raise FlowError(
                    "Recorded integration branch is registered at a different path."
                )
            else:
                self.runner.run(
                    [
                        ...
                        "--",
                        str(path),
                        state.integration_base_commit or state.base_branch,
                    ],
                    self.repo,
                )
    ...
    def _integration_plan(
        self, state: WorkflowState, feature_plan: Path, integration: Path
    ) -> Path:
        assert self.integration is not None
        self._verify_plan(state)
        ...
        if optional_regular_file(target, label="integration plan"):
            ...
            self.integration.copy_context(
                Path(state.feature_worktree), integration, feature_plan
            )
    ...
    def _write_conflict_context(self, state: WorkflowState, integration: Path) -> None:
        ...
        status = "\n".join(
            ...
            for record in self.git.status_records(integration)
        )
    ...
    def _merge_integration(
        self, state: WorkflowState, integration: Path
    ) -> WorkflowState:
        assert self.git is not None
        ...
        if state.stage is WorkflowStage.INTEGRATION_WORKTREE_CREATED:
            if (
                state.integration_feature_commit is None
                or self.git.branch_tip(state.feature_branch)
                != state.integration_feature_commit
            ):
                raise FlowError("Feature branch changed before integration merge.")
            self.git.require_clean_except_artifacts(
                integration, phase="Pre-merge integration"
            )
            args = (
                ["git", "merge", "--squash", state.feature_branch]
                if state.merge_mode == "squash"
                else ["git", "merge", "--no-ff", "--no-commit", state.feature_branch]
            )
            ...
            if result.returncode != 0:
                ...
                state = self._transition(
                    ...
                    integration_worktree_fingerprint=fingerprint,
                )
            else:
                fingerprint = self.git.fingerprint(integration)
                state = self._transition(
    ...
    def _verify_integration_checkpoint(
        self, state: WorkflowState, integration: Path
    ) -> None:
        assert self.git is not None
        actual = self.git.fingerprint(integration)
        if (
            state.integration_worktree_fingerprint is not None
            and actual != state.integration_worktree_fingerprint
        ):
            raise FlowError(
                "Integration worktree changed after the recorded checkpoint."
            )

    def _resolve_conflicts(
        self, state: WorkflowState, plan: Path, integration: Path
    ) -> WorkflowState:
        assert self.adapter is not None and self.git is not None
        if state.integration_branch is None:
        ...
        if state.stage is WorkflowStage.CONFLICT_RESOLUTION_STARTED:
            ...
            try:
                ...
                self.git.require_integration_worktree(
                    integration, state.integration_branch
                )
                state = self._change(
                    state,
                    integration_worktree_fingerprint=self.git.fingerprint(integration),
                )
                raise
            self.git.require_integration_worktree(integration, state.integration_branch)
            if self.git.head(integration) != head_before or self.git.has_unmerged(
                integration
            ):
                raise FlowError(
                    "Conflict resolution must clear unmerged entries without moving integration HEAD."
                )
        ...
        if state.stage is WorkflowStage.POST_CONFLICT_AUDIT_STARTED:
            ...
            try:
                self.adapter.execute(
                    integration,
                    audit_prompt(
                        integration, plan, self.harness_dir, post_conflict=True
                    ),
                    phase="post_conflict_audit",
                )
            except FlowError:
                self.git.require_integration_worktree(
                    integration, state.integration_branch
            ...
            if self.git.head(integration) != head_before or self.git.has_unmerged(
                ...
                raise FlowError(
                    "Post-conflict audit must not move integration HEAD or leave conflicts."
                )
    ...
    def _stage_integration(
        ...
        if (
            state.stage is WorkflowStage.INTEGRATION_MERGE_APPLIED
            or state.stage is WorkflowStage.POST_CONFLICT_AUDIT_COMPLETE
        ):
            self._verify_integration_checkpoint(state, integration)
    ...
    def _commit_integration(
        ...
        if state.stage is WorkflowStage.INTEGRATION_CHANGES_STAGED:
            ...
            if generated:
                self.git.require_clean_except_artifacts(
                    integration, phase="Completed integration commit"
                )
                state = self._transition(
                    state, WorkflowStage.INTEGRATION_COMMITTED, integration_commit=head
                )
                return state
            self._verify_integration_checkpoint(state, integration)
            self.git.require_no_unstaged_non_artifact(
                integration, phase="Integration before commit"
            )
            self.integration.require_staged_changes(integration)
            ...
            if (
                ...
                raise FlowError(
                    "Integration commit receipt does not match the required parent shape."
                )
            state = self._transition(
                state, WorkflowStage.INTEGRATION_COMMITTED, integration_commit=commit
            )
            ...
            self.git.require_commit_oid(
                state.integration_commit, label="integration commit"
            )
            if self.git.head(integration) != state.integration_commit:
                raise FlowError(
                    "Integration HEAD differs from integration_commit receipt."
                )
        return state
    ...
    def _fast_forward(self, state: WorkflowState, integration: Path) -> WorkflowState:
        ...
        self.git.require_primary_ready(state.base_branch)
        if (
            state.integration_branch is None
        ...
        self.git.require_clean_except_artifacts(
            feature_worktree, phase="Feature before fast-forward"
        )
        self.git.require_integration_worktree(integration, state.integration_branch)
        if (
            self.git.head(integration) != state.integration_commit
            or self.git.branch_tip(state.integration_branch) != state.integration_commit
        ):
            raise FlowError(
                "Integration HEAD or branch differs from integration_commit receipt."
            )
        self.git.require_clean_except_artifacts(
            integration, phase="Integration before fast-forward"
        )
        self._verify_plan(state)
        ...
        if (
            ...
            raise FlowError(
                "Feature HEAD changed after the recorded integration receipt."
            )
        if (
            self.git.commit_subject(state.integration_commit, integration)
            != f"Harness: {state.plan_title}"
        ):
            raise FlowError(
                "Integration commit subject is not generated by this workflow."
            )
        parents = self.git.commit_parents(state.integration_commit, integration)
        if state.merge_mode == "squash":
            expected_parents = (state.integration_base_commit,)
        elif (
            state.merge_mode == "no-ff" and state.integration_feature_commit is not None
        ):
            expected_parents = (
                state.integration_base_commit,
                state.integration_feature_commit,
            )
        else:
        ...
        if parents != expected_parents:
            raise FlowError(
                "Integration commit parents differ from the recorded merge receipt."
            )
        current_base = self.git.branch_tip(state.base_branch)
        if current_base != state.integration_base_commit:
            if self.git.merge_base_is_ancestor(
                state.integration_base_commit, current_base
            ):
                state = self._transition(
                    state, WorkflowStage.INTEGRATION_REBUILD_CLEANUP
                )
        ...
        if state.stage is WorkflowStage.INTEGRATION_COMMITTED:
            ...
            if not self.git.base_contains(state.integration_commit, state.base_branch):
                raise FlowError(
                    "Primary base does not contain the integration commit after fast-forward."
                )
            state = self._transition(state, WorkflowStage.BASE_FAST_FORWARDED)
        return state

    def _cleanup_generated_integration(
        self, state: WorkflowState, integration: Path
    ) -> None:
        assert self.git is not None
        self.git.require_primary_ready(state.base_branch)
        ...
        if entries:
            ...
            self.git.require_clean_except_artifacts(
                target, phase="Integration rebuild cleanup"
            )
        ...
        self._remove_or_adopt_worktree(
            ...
            state.integration_branch,
            allow_adopt=True,
            branch_may_be_absent=True,
        )
        if self.git.branch_exists(state.integration_branch):
            self.git.delete_branch(state.integration_branch, force=True)

    def _archive(
        self, state: WorkflowState, source: Path, plan: Path
    ) -> tuple[WorkflowState, Path]:
        assert (
            self.git is not None
        ...
        self.git.require_feature_worktree(feature_worktree, state.feature_branch)
        self.git.require_clean_except_artifacts(
            feature_worktree, phase="Feature before archive"
        )
        if (
            state.audit_head is None
            or self.git.head(feature_worktree) != state.audit_head
        ):
            raise FlowError("Feature HEAD changed before archive.")
        self.git.require_integration_worktree(source, state.integration_branch)
        if (
            self.git.head(source) != state.integration_commit
            or self.git.branch_tip(state.integration_branch) != state.integration_commit
        ):
            raise FlowError(
                "Integration HEAD or branch differs from integration_commit receipt."
        ...
        self.git.require_clean_except_artifacts(
            source, phase="Integration before archive"
        )
        archive = self._archive_candidate(state.run_id)
        self.integration.archive_handoff(source, archive, plan)
        state = self._transition(
            state,
            WorkflowStage.HANDOFF_ARCHIVED,
            archive_dir=str(canonical_path(archive, must_exist=True)),
        )
        self.integration.write_candidate_state(archive, state)
        return state, archive

    def _archive_receipt(
        self, commit: str, archive: Path, relative: str, expected_parent: str
    ) -> None:
        ...
        if actual_paths != expected_paths:
            raise FlowError(
                "Archive commit receipt does not match the exact archive manifest."
            )
        if not self.git.commit_touches(commit, relative, self.repo):
            raise FlowError(
                "Archive commit receipt does not touch the generated archive."
            )
    ...
    def _commit_archive(self, state: WorkflowState, archive: Path) -> WorkflowState:
        ...
        if state.integration_commit is None:
            raise FlowError(
                "Integration commit receipt is missing before archive commit."
            )
        relative = archive.relative_to(self.repo).as_posix()
        ...
        head = self.git.head(self.repo)
        if (
            state.stage is WorkflowStage.HANDOFF_ARCHIVED
            and self.git.commit_subject(head, self.repo) == subject
        ):
            self._archive_receipt(head, archive, relative, state.integration_commit)
            return self._transition(
                state, WorkflowStage.ARTIFACTS_COMMITTED, archive_commit=head
            )
        self.integration.exact_manifest(archive)
        self.runner.run(["git", "add", "-A", "--", literal_relative], self.repo)
        diff = self.runner.run(
            ["git", "diff", "--cached", "--quiet", "--", literal_relative],
            self.repo,
            check=False,
        )
        if diff.returncode == 1:
            self.runner.run(
                ["git", "commit", "-m", subject, "--", literal_relative], self.repo
            )
            commit = self.git.head(self.repo)
            if self.git.commit_subject(commit, self.repo) != subject:
                raise FlowError(
                    "Archive commit subject does not match the generated archive."
                )
            self._archive_receipt(commit, archive, relative, state.integration_commit)
            state = self._transition(
                state, WorkflowStage.ARTIFACTS_COMMITTED, archive_commit=commit
            )
    ...
    def _remove_or_adopt_worktree(
        ...
        *,
        ...
        if (
            ...
            and (branch_may_be_absent or self.git.branch_exists(branch))
        ):
            return
        raise FlowError(
            f"Generated worktree is not registered at the recorded path: {target}"
        )
    ...
    def _cleanup_stage(self, state: WorkflowState, integration: Path) -> WorkflowState:
        ...
        if state.stage is WorkflowStage.ARTIFACTS_COMMITTED:
            ...
            if (
                self.git.branch_exists(state.integration_branch)
                and self.git.branch_tip(state.integration_branch)
                != state.integration_commit
            ):
            ...
            if any(entry.path == integration for entry in self.git.worktrees()):
                ...
                self.git.require_clean_except_artifacts(
                    integration, phase="Integration before cleanup"
                )
            self._remove_or_adopt_worktree(
                integration, state.integration_branch, allow_adopt=True
            )
        ...
        if state.stage is WorkflowStage.INTEGRATION_WORKTREE_REMOVED:
            ...
            if (
                self.git.branch_exists(state.feature_branch)
                and self.git.branch_tip(state.feature_branch) != state.audit_head
            ):
            ...
            if any(entry.path == feature for entry in self.git.worktrees()):
                ...
                self.git.require_clean_except_artifacts(
                    feature, phase="Feature before cleanup"
                )
            self._remove_or_adopt_worktree(
                feature, state.feature_branch, allow_adopt=True
            )
        ...
        if state.stage is WorkflowStage.WORKTREES_PRUNED:
            if state.integration_branch and self.git.branch_exists(
                state.integration_branch
            ):
                if (
                    state.integration_commit is None
                    or self.git.branch_tip(state.integration_branch)
                    != state.integration_commit
                ):
                    raise FlowError("Integration branch changed before cleanup.")
                self.git.delete_branch(state.integration_branch, force=True)
        ...
        if state.stage is WorkflowStage.INTEGRATION_BRANCH_REMOVED:
            if self.git.branch_exists(state.feature_branch):
                if (
                    state.audit_head is None
                    or self.git.branch_tip(state.feature_branch) != state.audit_head
                ):
                    raise FlowError("Feature branch changed before cleanup.")
    ...
    def _finalize(self, state: WorkflowState, archive: Path) -> WorkflowState:
        ...
        if state.stage not in {
            ...
            WorkflowStage.WORKTREES_RETAINED,
        }:
            return state
        ...
        if self.git.commit_subject(head, self.repo) == subject:
            try:
                candidate = self.state_store.read_candidate_state(
                    archive / WORKFLOW_STATE_FILENAME
                )
            except FlowError:
                candidate = None
            if (
                candidate is not None
                and candidate.run_id == state.run_id
                and candidate.stage is WorkflowStage.COMPLETE
            ):
                self._archive_receipt(head, archive, relative, state.archive_commit)
                state = self._change(state, final_state_commit=head)
                return self._transition(
                    state, WorkflowStage.COMPLETE, final_state_commit=head
                )
        self.integration.write_candidate_state(
            archive,
            replace(state, stage=WorkflowStage.COMPLETE, final_state_commit=None),
        )
        self.git.require_primary_ready(state.base_branch)
        self.runner.run(["git", "add", "-A", "--", literal_relative], self.repo)
        self.runner.run(
        ...
        if self.git.commit_subject(commit, self.repo) != subject:
            raise FlowError(
                "Final-state commit subject does not match the generated archive."
            )
        self._archive_receipt(commit, archive, relative, state.archive_commit)
        state = self._change(state, final_state_commit=commit)
        state = self._transition(
            state, WorkflowStage.COMPLETE, final_state_commit=commit
        )
        return state
    ...
    def _finish(self, state: WorkflowState, feature_plan: Path) -> WorkflowState:
        ...
        while state.stage is not WorkflowStage.COMPLETE:
            if state.stage is WorkflowStage.INTEGRATION_REBUILD_CLEANUP:
                if (
                    state.integration_base_commit is None
                    or state.integration_commit is None
                ):
                    raise FlowError(
                        "Integration receipt is incomplete during rebuild cleanup."
                    )
                current_base = self.git.branch_tip(state.base_branch)
                if (
                    current_base == state.integration_base_commit
                    or not self.git.merge_base_is_ancestor(
                        state.integration_base_commit, current_base
                    )
                ):
                    raise FlowError(
                        "Primary base changed incompatibly before integration rebuild cleanup."
                    )
            ...
            if state.stage in allocation_stages:
                ...
                integration = Path(state.integration_worktree or "")
            if state.stage is WorkflowStage.INTEGRATION_WORKTREE_CREATED:
                integration_plan = self._integration_plan(
                    state, feature_plan, integration
                )
                ...
                if state.stage in {
                    ...
                    state = self._resolve_conflicts(
                        state, integration_plan, integration
                    )
                ...
                archive = Path(
                    state.archive_dir or self._archive_candidate(state.run_id)
                )
                state = self._commit_archive(state, archive)
                ...
                raise FlowError(
                    f"Cannot dispatch integration stage {state.stage.value}."
                )
        return state

    def _commit_stop_archive(
        self, state: WorkflowState, archive: Path
    ) -> WorkflowState:
        assert self.git is not None and self.integration is not None
        self.git.require_primary_ready(state.base_branch)
        relative = archive.relative_to(self.repo).as_posix()
        ...
        if self.git.commit_subject(commit, self.repo) != subject:
            raise FlowError(
                "Stopped archive commit subject does not match the generated archive."
            )
    ...
    def run(self) -> None:
        ...
        with self.state_store.lock(names.run_id):
            ...
            self.print_checkpoint(
                "start",
                "Worktree flow",
                (("plan", plan), ("base", self.base), ("merge mode", state.merge_mode)),
            )
            self.print_checkpoint(
                "ready",
                "Feature target",
                (
                    ("branch", names.feature_branch),
                    ("worktree", names.feature_worktree),
                    ("state", run_dir),
                ),
            )
            ...
            if state.merge_mode == "stop":
                ...
                archive = self._archive_candidate(state.run_id)
                assert self.integration is not None
                self.integration.archive_handoff(
                    Path(state.feature_worktree), archive, plan_in_worktree
                )
                state = self._change(
                    state, archive_dir=str(canonical_path(archive, must_exist=True))
                )
                self.integration.write_candidate_state(archive, state)
                ...
                self.print_checkpoint(
                    ...
                    (("archive", archive), ("worktree", state.feature_worktree)),
                )
                return
            state = self._finish(state, plan_in_worktree)
            self.print_checkpoint(
                "done", "Worktree flow", (("stage", state.stage.value),)
            )
    ...
    def _validate_complete_receipts(self, state: WorkflowState) -> None:
        ...
        if self.git.commit_subject(state.archive_commit, self.repo) != archive_subject:
            raise FlowError(
                "Archive commit receipt subject does not match the generated archive."
            )
        if self.git.commit_parents(state.archive_commit, self.repo) != (
            state.integration_commit,
        ):
            raise FlowError(
                "Archive commit receipt parent does not match integration_commit."
            )
        self._archive_receipt(
            state.archive_commit, archive, relative, state.integration_commit
        )
        final_subject = f"Harness: finalize {state.plan_title}"
        if (
            self.git.commit_subject(state.final_state_commit, self.repo)
            != final_subject
        ):
            raise FlowError(
                "Final-state commit receipt subject does not match the generated archive."
            )
        if self.git.commit_parents(state.final_state_commit, self.repo) != (
            state.archive_commit,
        ):
            raise FlowError(
                "Final-state commit receipt parent does not match archive_commit."
            )
        self._archive_receipt(
            state.final_state_commit, archive, relative, state.archive_commit
        )

    def _validate_resume_state(
        self, state: WorkflowState, requested_worktree: Path | None
    ) -> None:
        assert self.git is not None and self.state_store is not None
        if requested_worktree is not None and canonical_path(
            requested_worktree, must_exist=False
        ) != Path(state.feature_worktree):
            raise FlowError(
                "Explicit --worktree does not match the recorded feature worktree."
            )
        ...
        if state.stage is WorkflowStage.COMPLETE:
            ...
            if state.archive_dir is None or canonical_path(
                Path(state.archive_dir), must_exist=False
            ) != canonical_path(expected_archive, must_exist=False):
                raise FlowError(
                    "Completed workflow archive path does not match its run id."
                )
        ...
        expected_feature = (
            self.repo.parent / f"{self.repo.name}-{state.slug}"
        ).resolve()
        if feature_path != expected_feature:
            raise FlowError(
                "Recorded feature worktree path is not generated for this run."
            )
        if state.feature_branch != f"feature/{state.slug}":
            raise FlowError("Recorded feature branch is not generated for this run.")
        if state.archive_dir is not None and canonical_path(
            Path(state.archive_dir), must_exist=False
        ) != self._archive_candidate(state.run_id):
            raise FlowError("Recorded archive path does not match its run id.")
        if (
            state.stage is not WorkflowStage.FEATURE_ALLOCATED
            and state.stage in feature_removed_stages
        ):
            entries = [
                entry for entry in self.git.worktrees() if entry.path == feature_path
            ]
            if entries:
                ...
                raise FlowError(
                    "Recorded feature path exists without its registered worktree."
                )
                ...
                raise FlowError(
                    "Recorded feature worktree disappeared before its checkpoint."
                )
        ...
        if state.stage not in {
            ...
            *feature_removed_stages,
            ...
            if canonical_path(expected_plan, must_exist=False) != Path(state.plan_path):
                raise FlowError(
                    "Recorded plan path does not match the generated feature plan path."
                )
            self._verify_plan(state)
        if (
            state.integration_worktree is not None
            or state.integration_branch is not None
        ):
            ...
            if (
                re.fullmatch(
                    rf"integration/{re.escape(state.slug)}-\d{{8}}-\d{{6}}(?:-\d+)?",
                    state.integration_branch,
                )
                is None
            ):
                raise FlowError(
                    "Recorded integration branch is not generated for this run."
                )
            integration_path = canonical_path(
                Path(state.integration_worktree), must_exist=False
            )
            if (
                integration_path.parent != self.repo.parent
                or re.fullmatch(
                ...
                raise FlowError(
                    "Recorded integration worktree path is not generated for this run."
                )
            ...
            if state.stage not in integration_missing_stages:
                self.git.require_integration_worktree(
                    integration_path, state.integration_branch
                )
            else:
                entries = [
                    entry
                    for entry in self.git.worktrees()
                    if entry.path == integration_path
                ]
                if entries:
                    if entries[0].branch != state.integration_branch:
                        raise FlowError(
                            "Recorded integration allocation is mismatched."
                        )
                elif integration_path.exists():
                    raise FlowError(
                        "Recorded integration path exists without its registered worktree."
                    )
                elif state.stage not in {
                    WorkflowStage.INTEGRATION_REBUILD_CLEANUP,
                    WorkflowStage.WORKTREES_PRUNED,
                    ...
                    raise FlowError(
                        "Recorded integration worktree disappeared before its checkpoint."
                    )

    def _find_resume_run(
        self, plan: Path, requested_worktree: Path | None
        ...
        if len(candidates) != 1:
            raise FlowError(
                "Implicit resume requires exactly one matching schema-v2 workflow state; pass --worktree."
            )
        return candidates[0]
    ...
    def _apply_resume_configuration(self, state: WorkflowState) -> WorkflowState:
        ...
        if (
            self.config.harness_dir_explicit
            and self.harness_dir.as_posix() != state.harness_dir
        ):
            raise FlowError("Persisted harness directory is authoritative on resume.")
        if (
            self.config.keep_worktrees_explicit
            and self.config.keep_worktrees != state.keep_worktrees
        ):
            raise FlowError(
                "Persisted keep-worktrees setting is authoritative on resume."
            )
        ...
        if self.config.model_explicit:
            ...
            review_model = (
                self.config.review_model if self.config.review_model_explicit else model
            )
        else:
            if self.config.implementation_model_explicit:
        ...
        with self.state_store.lock(run_id):
            ...
            if state.stage is WorkflowStage.COMPLETE:
                self.print_checkpoint(
                    ...
                    (
                        ("run id", state.run_id),
                        ("final state commit", state.final_state_commit),
                    ),
                )
                return
    ...
    def resume_command_args(self) -> list[str] | None:
        ...
        args = [
            sys.executable,
            str(self.config.entrypoint_path.resolve()),
            "--resume",
            "--plan",
            state.plan_path,
            "--repo",
            ...
            state.base_branch,
            "--state-dir",
            str(self.state_store.root if self.state_store else ""),
        ]
        if state.merge_mode != "stop":
        ...
        if state.command_timeout_seconds is not None:
            args.extend(
                ["--command-timeout-seconds", str(state.command_timeout_seconds)]
            )
        if state.keep_worktrees:
    ...
    def preview(self) -> None:
        ...
        implementation_selector = phase_model_for_preview(
            kind, "implementation", self.config
        )
        ...
        print(
            f"+ implementation: {self._preview_harness(feature, 'implementation', implementation_selector)}"
        )
        ...
        print(
            f"+ conflict resolution: {self._preview_harness(integration, 'conflict_resolution', implementation_selector)}"
        )
        print(
            f"+ post-conflict audit: {self._preview_harness(integration, 'post_conflict_audit', audit_selector)}"
        )
        print(f"+ integration commit: git commit -m 'Harness: {title}'")
        print(f"+ fast-forward: git merge --ff-only {integration_branch}")
        print(
            f"+ archive: copy allowlisted handoff artifacts to {self.repo / self.worktree_flow_dir / '<run-id>'}"
        )
        print(
            f"+ cleanup: git worktree remove --force {integration}; git worktree remove --force {feature}; git worktree prune; git branch -D"
        )
        ...
        if kind is HarnessKind.OMP:
            ...
            args.extend(
                [
                    "--file",
                    str(prompt),
                    "Execute the attached worktree-flow phase prompt.",
                ]
            )
        return shell_command(args)


def phase_model_for_preview(
    kind: HarnessKind, phase: str, config: FlowConfig
) -> str | None:
    if phase in {"audit", "post_conflict_audit"}:
        return (
            config.review_model
            or config.model
            or ("@slow" if kind is HarnessKind.OMP else None)
        )
    return (
