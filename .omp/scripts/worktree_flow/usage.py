...
class UsageCollector:
    def __init__(
        self, *, harness: str, harness_dir: Path, dry_run: bool = False
    ) -> None:
    ...
    def snapshot_sessions(self, repo: Path) -> SessionSnapshot:
        return SessionSnapshot(
            {
                str(path.resolve()): self.safe_mtime_ns(path)
    ...
    def wsl_windows_home_from_repo(self, repo: Path) -> Path | None:
        match = re.match(
            r"^/mnt/([A-Za-z])/Users/([^/]+)(?:/|$)", repo.resolve().as_posix()
        )
        if match is None:
            return None
    ...
    def changed_session_files(
        self, repo: Path, snapshot: SessionSnapshot
    ) -> list[Path]:
    ...
    def read_jsonl_records(self, path: Path) -> Iterator[JsonObject]:
        for raw in iter_bounded_jsonl(path, max_line_bytes=MAX_JSONL_LINE_BYTES):
            ...
            try:
                value = json.loads(
                    raw.decode("utf-8"), parse_constant=self._reject_json_constant
                )
    ...
    def session_cwd(self, path: Path) -> str | None:
        for record in self.read_jsonl_records(path):
            if record.get("type") not in {"session", "session_init"}:
                continue
            cwd = self.string_at(record, ("cwd",)) or self.string_at(
                record, ("data", "cwd")
            )
            if cwd:
                return cwd
    ...
    def wsl_drive_mount_to_windows_path(path: Path) -> str | None:
        ...
        tail_windows = tail.replace("/", "\\") if tail else ""
        return f"{drive.upper()}:\\{tail_windows}".rstrip("\\")
    ...
    def numeric_at(
        self, data: Mapping[str, Any], path: tuple[str, ...]
    ) -> Number | None:
        value: Any = data
        for key in path:
    ...
    @staticmethod
    def increment(
        target: MutableMapping[str, Any], key: str, amount: Number = 1
    ) -> None:
    ...
    def collect_config(self, record: JsonObject, aggregate: UsageAggregate) -> bool:
        ...
        for record_type, path, metric in (
            ...
            (
                ...
                "thinking_level_selections",
            ),
            (
                "service_tier_change",
                ("service_tier_change", "serviceTier"),
                "service_tier_selections",
            ),
        ):
            if record.get("type") != record_type:
                continue
            value = self.string_at(record, path) or self.string_at(
                record, ("data", path[-1])
            )
            if value:
    ...
    def collect_context(self, message: JsonObject, aggregate: UsageAggregate) -> bool:
        ...
        for source, last_key, max_key in (
            ...
            if value is not None:
                ...
                aggregate.context[max_key] = max(
                    int(aggregate.context.get(max_key, 0)), int(value)
                )
                contributed = True
        return contributed
    ...
    def collect_tool(self, record: JsonObject, aggregate: UsageAggregate) -> bool:
        if record.get("customType") == "tool_execution_start":
            ...
            if tool_name:
                bucket = aggregate.tools.setdefault(
    ...
    ) -> tuple[UsageAggregate, UsageSource]:
    ...
    def collect_phase_usage(
        ...
        selected = [
            ...
            if (cwd := self.session_cwd(path))
            and self.path_matches_worktree(cwd, worktree)
        ]
        if not selected:
            event.update(
                {"status": "unavailable", "reason": "no_matching_session_files"}
            )
            return event
        aggregate = UsageAggregate.empty()
        sources: list[JsonObject] = []
        for index, path in enumerate(selected, start=1):
            source_aggregate, source = self.collect_session_usage(
                path, f"session-{index}"
            )
            self.merge_aggregate(aggregate, source_aggregate)
            sources.append(asdict(source))
        event.update(
            {"status": "collected", **aggregate.as_payload(), "sources": sources}
        )
        return event
    ...
    def merge_aggregate(self, target: UsageAggregate, source: UsageAggregate) -> None:
        ...
        for key in ("max_prompt_tokens", "max_non_message_tokens"):
            target.context[key] = max(
                int(target.context.get(key, 0)), int(source.context.get(key, 0))
            )
    ...
    def merge_models(
        self, target: MutableMapping[str, JsonObject], source: Mapping[str, Any]
    ) -> None:
    ...
    def append_event(self, worktree: Path, event: JsonObject) -> None:
        ...
        previous = (
            read_bytes_bounded(path, max_bytes=10 * 1024 * 1024)
            if optional_regular_file(path, label="usage events")
            else b""
        )
    ...
    def rewrite_artifacts(self, worktree: Path) -> None:
        ...
        events = (
            list(self.read_jsonl_records(events_path))
            if optional_regular_file(events_path, label="usage events")
            else []
        )
        ...
        for event in events:
            ...
            if isinstance(source_values, list):
                path_hashes.update(
                    ...
                    and isinstance(item.get("path_hash"), str)
                )
