...
class GitWorkspace:
    ...
    def require_registered_worktree(
        self, path: Path, expected_branch: str
    ) -> GitWorktree:
        ...
        for entry in self.worktrees():
            ...
            if target.exists() and self.git_common_dir(target) != common:
                raise FlowError(
                    f"Worktree {target} belongs to a different Git repository."
                )
            return entry
        raise FlowError(
            f"Worktree is not registered in the selected repository: {target}"
        )

    def current_branch(self, worktree: Path | None = None) -> str:
        result = self._run(
            ["git", "branch", "--show-current"], worktree or self.repo, check=False
        )
        if result.returncode != 0:
            raise FlowError(
                f"Cannot determine current branch for {worktree or self.repo}"
            )
        return result.stdout.strip()

    def head(self, worktree: Path | None = None) -> str:
        result = self._run(
            ["git", "rev-parse", "--verify", "HEAD^{commit}"], worktree or self.repo
        )
    ...
    def require_commit_oid(self, value: str, *, label: str = "commit") -> str:
        if (
            ...
            raise FlowError(
                f"Invalid {label} receipt; expected a canonical full commit OID."
            )
        result = self._run(
            [
                "git",
        ...
        if result.returncode != 0:
            raise FlowError(
                f"{label} receipt does not name an existing commit: {value}"
            )
        canonical = result.stdout.strip().lower()
        if canonical != value:
            raise FlowError(
                f"{label} receipt is not the repository's canonical full OID: {value}"
            )
        return canonical
    ...
    def validate_local_branch_name(self, name: str) -> str:
        ...
        check = self._run(
            ["git", "check-ref-format", "--branch", name], self.repo, check=False
        )
    ...
    def primary_entry(self) -> GitWorktree:
        ...
        if primary.path != self.repo or primary.bare:
            raise FlowError(
                "Selected repository is not the primary Git worktree entry."
            )
        return primary
    ...
    def tracked_handoff_paths(
        self, worktree: Path | None = None, treeish: str = "HEAD"
    ) -> list[str]:
        result = self._run(
            [
                "git",
    ...
    def require_no_tracked_handoff(
        self, worktree: Path | None = None, treeish: str = "HEAD"
    ) -> None:
        tracked = self.tracked_handoff_paths(worktree, treeish)
        if tracked:
            raise FlowError(
                "Tracked handoff artifacts are not allowed: " + ", ".join(tracked)
            )

    def non_artifact_status(self, worktree: Path | None = None) -> list[StatusRecord]:
        return [
            record
            for record in self.status_records(worktree)
            if not self.path_is_artifact(record.path)
            and not (
                record.original_path and self.path_is_artifact(record.original_path)
            )
        ]

    def require_clean_except_artifacts(
        self, worktree: Path | None = None, *, phase: str = "Workflow"
    ) -> None:
        ...
        if records:
            ...
            raise FlowError(
                f"{phase} left non-artifact changes in {worktree or self.repo}:\n"
                + "\n".join(details)
            )
    ...
    def require_primary_ready(self, base_branch: str) -> None:
        ...
        if self.current_branch() != base_branch:
            raise FlowError(
                f"Primary repository must already be checked out on {base_branch}."
            )
    ...
    def count_commits_since(
        self, base: str, branch: str, worktree: Path | None = None
    ) -> int:
        result = self._run(
            ["git", "rev-list", "--count", "--end-of-options", f"{base}..{branch}"],
            worktree or self.repo,
        )
        try:
    ...
    def merge_base_is_ancestor(
        self, ancestor: str, descendant: str, worktree: Path | None = None
    ) -> bool:
        result = self._run(
            [
                "git",
    ...
    def unmerged_paths(self, worktree: Path) -> list[str]:
        ...
        if result.returncode != 0:
            raise FlowError(format_command_failure(result))
        paths = {
            field.split("\t", 1)[-1] for field in result.stdout.split("\0") if field
        }
        return sorted(paths)
    ...
    def commit_paths(
        self, commit: str, relative_path: str, worktree: Path | None = None
    ) -> tuple[str, ...]:
        if Path(relative_path).is_absolute() or any(
            part in {"", ".", ".."} for part in Path(relative_path).parts
        ):
            raise FlowError(
                "Commit path must be a normalized repository-relative path."
            )
        result = self._run(
            [
                "git",
    ...
    def commit_touches(
        ...
        return any(
            line == relative_path or line.startswith(relative_path + "/")
            for line in result.stdout.splitlines()
        )

    def commit_parents(
        self, commit: str, worktree: Path | None = None
    ) -> tuple[str, ...]:
        result = self._run(
            ["git", "show", "-s", "--format=%P", "--end-of-options", commit],
            worktree or self.repo,
        )
    ...
    def commit_tree(self, commit: str, worktree: Path | None = None) -> str:
        result = self._run(
            ["git", "show", "-s", "--format=%T", "--end-of-options", commit],
            worktree or self.repo,
        )
        return result.stdout.strip()
    ...
    def require_feature_worktree(self, path: Path, branch: str) -> GitWorktree:
        ...
        if self.current_branch(entry.path) != branch:
            raise FlowError(
                f"Feature worktree is not checked out on {branch}: {entry.path}"
            )
        return entry
    ...
    def parse_untracked_artifacts(self, worktree: Path) -> list[str]:
        return [
            record.path
            for record in self.status_records(worktree)
            if record.code == "??" and self.path_is_artifact(record.path)
        ]
    ...
    def list_refs(self, prefix: str) -> list[str]:
        result = self._run(
            ["git", "for-each-ref", "--format=%(refname)", prefix], self.repo
        )
    ...
    def require_same_repository(self, path: Path) -> None:
        if self.git_common_dir(path) != self.git_common_dir(self.repo):
            raise FlowError(
                f"Path is not registered in the selected Git repository: {path}"
            )
    ...
    def assert_branch_tip(self, branch: str, expected: str) -> None:
        ...
        if actual != expected:
            raise FlowError(
                f"Branch {branch} tip changed unexpectedly: {actual} != {expected}"
            )
