...
class TestDriveAPI:
    ...
    def test_find_folders_returns_folder_metadata(self) -> None:
        api = DriveAPI(credentials_path="creds.json", token_path="token.json")
        mock_service = MagicMock()
        mock_service.files().list().execute.return_value = {
            "files": [{"id": "folder_1", "name": "Profile Backup", "parents": ["root"]}]
        }
        api._service = mock_service

        folders = api.find_folders("Profile Backup")

        assert len(folders) == 1
        assert folders[0].id == "folder_1"
        assert folders[0].parents == ["root"]

    def test_rename_and_move_folder_updates_drive(self) -> None:
        api = DriveAPI(credentials_path="creds.json", token_path="token.json")
        mock_service = MagicMock()
        ...
        result = api.rename_and_move_folder(
            folder_id="folder_1",
            new_name="laptop-a",
            new_parent_id="parent_1",
            old_parent_ids=["root"],
        )
